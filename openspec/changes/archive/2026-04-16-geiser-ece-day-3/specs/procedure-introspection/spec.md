...
ECE SHALL provide a `%procedure-params` host primitive that, given a callable value, returns its parameter metadata as `(param-names . rest?)` where `param-names` is a list of strings and `rest?` is a boolean. For host primitives, it SHALL return arity-based metadata. If no metadata is available, it SHALL return `#f`.
