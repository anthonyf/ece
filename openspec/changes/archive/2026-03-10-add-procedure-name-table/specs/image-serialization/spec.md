...
The `save-image!` primitive SHALL write the complete ECE system state to a file. The state includes: the global instruction source vector, the global label table, the global environment, the compile-time macro table, and the procedure name table.
...
The `load-image!` primitive SHALL read an image file and replace all global state. After loading, the system SHALL behave identically to the state at save time, including procedure name mappings.
