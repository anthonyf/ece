...
The test suite SHALL verify division via the evaluator. Primitives SHALL be stored by symbol name and resolved at call time.
...
The test suite SHALL verify `=`, `<`, `>`, `<=`, `>=` via the evaluator. Primitives SHALL be stored by symbol name and resolved at call time.
...
The test suite SHALL verify `car`, `cdr`, `cons`, `list`, `null?`, and `not` via the evaluator. Primitives SHALL be stored by symbol name and resolved at call time.
