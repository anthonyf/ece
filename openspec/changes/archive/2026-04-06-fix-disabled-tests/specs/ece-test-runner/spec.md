...
The `make test-rove` target SHALL use the `rove:run` API to run tests and check pass/fail status. It MUST exit 0 when all tests pass and exit 1 when any test fails. It MUST NOT reference internal rove symbols that may not exist across rove versions.
