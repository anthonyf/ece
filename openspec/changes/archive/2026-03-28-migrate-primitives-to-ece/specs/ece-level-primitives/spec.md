...
Migrated primitives SHALL have their platform annotation changed from `core` to `ece` in `primitives.def` to document that they are now prelude-level.

#### Scenario: annotation reflects migration
- **WHEN** `primitives.def` is read
- **THEN** `char-whitespace?` (47), `char-alphabetic?` (48), `char-numeric?` (49), `equal?` (21), `eqv?` (174), `gensym` (82), `string-downcase` (36), `string-upcase` (37), `string-split` (38), `string-trim` (39), `string-contains?` (40), `string-join` (41), and `print` (66) SHALL have platform `ece`
