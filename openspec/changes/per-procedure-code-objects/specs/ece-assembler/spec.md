...
**Reason:** The CL `assemble-into-global` function retires along with the shared global instruction vector. With pure per-code-object assembly, there is no "append to global" operation to compare against. The new requirement is that the assembler produces code objects; equivalence is defined by execution behavior, not by byte-for-byte vector parity with a retired function.

**Migration:** No caller migration required beyond the general pivot from space-based to code-object-based compilation. Regression coverage shifts to end-to-end execution tests (run the compiled result and assert behavior) rather than structural tests on a shared vector.
