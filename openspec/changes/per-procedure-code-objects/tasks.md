...
- [ ] 6.6 Parallel WASM executor changes in `wasm/runtime.wat`: `$current-space-id` → `$current-code-obj` (struct pointer). `$switch-space` → `$switch-code-obj`. Remove the `get-space` equivalent.
      *Pass 1 done (infrastructure, no behaviour change): `$compiled-proc` gained a `$code-obj: (ref null eq)` field; `%make-compiled-procedure` routes a bare code-object entry into that field. All existing tests pass unchanged.*
      *Pass 2 pending — budgeted as a dedicated session. Three blocks of ~100 LOC each in `wasm/runtime.wat`: (a) `$execute` must read `$instrs`/`$labels`/`$len` from a code-object when the current `$compiled-proc.$code-obj` is non-null; parallel state means adding `$current-code-obj: (ref null eq)` alongside `$space-id: i32` and updating ~40 touchpoints. (b) `goto (reg ...)` must handle bare code-object and `(code-obj . pc)` addrs (currently ref.casts car to i31 and traps on code-objects). (c) `%code-object-push-instruction!` needs a parallel parser that resolves labels against the code-object's own `$labels` hash instead of a `$comp-space`, plus a per-code-object pending-list mechanism to handle forward labels (existing `$pending-instrs` flush is space-keyed). Strategy confirmed: coexistence via parallel fields — space-based boot path stays intact, code-object path opt-in from ECE.*
