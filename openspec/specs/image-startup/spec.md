...
ECE SHALL be able to start from `runtime.lisp` plus `boot.lisp` plus a saved image file, without loading `compiler.lisp`. The image provides all compiled state (prelude, metacircular compiler, reader, assembler). The `boot.lisp` file loads the image and provides `evaluate`, `ece-try-eval`, and `repl`. The `image-repl` function in runtime.lisp is removed in favor of `repl` in boot.lisp.
