...
This document describes the current ECE language and virtual-machine design as implemented in `main`. It consolidates the durable architecture notes for the live source tree.
...
| [`src/ece-main.scm`](../../src/ece-main.scm) | CLI/tool dispatcher, REPL, Geiser response mode, module entry-point execution, and installed SDK command behavior. |
| [`src/ece-build.scm`](../../src/ece-build.scm) | Application packaging for CL and web targets. |
| [`src/ece-test.scm`](../../src/ece-test.scm) | Self-hosted ECE test discovery, isolation, assertions, filtering, and reporting. |
| [`src/ece-serve.scm`](../../src/ece-serve.scm) | CL-hosted live browser development server implemented mostly in ECE. |
| [`src/scheduler.scm`](../../src/scheduler.scm) | Cooperative scheduler built on continuations, used by browser development and app code. |
| [`src/http-codec.scm`](../../src/http-codec.scm), [`src/websocket-codec.scm`](../../src/websocket-codec.scm), [`src/json.scm`](../../src/json.scm) | HTTP/WebSocket/JSON helpers used by browser development tooling and reusable modules. |
| [`src/browser-lib.scm`](../../src/browser-lib.scm), [`src/browser-dom.scm`](../../src/browser-dom.scm), [`src/browser-html.scm`](../../src/browser-html.scm), [`src/browser-canvas.scm`](../../src/browser-canvas.scm), [`src/browser-dev.scm`](../../src/browser-dev.scm), [`src/wasm-host.scm`](../../src/wasm-host.scm) | Browser-facing libraries over JavaScript FFI, DOM, HTML rendering, canvas, live update policy, and WASM host resources. |
| [`src/geiser-ece.scm`](../../src/geiser-ece.scm) | Scheme-side helper procedures for the Emacs Geiser backend. |
...
The stable user-facing language surface is summarized in the top-level
[`README.md`](../../README.md). Architecturally, most non-core Scheme forms and
libraries are ECE source layered over a small primitive and operation boundary.

Core compiler forms include `lambda`, `if`, `begin`, `define`, `set!`, `quote`,
`apply`, `%raw-call/cc`, `define-macro`, `let-syntax`, `letrec-syntax`,
`quasiquote`, and `%global-ref`. Derived forms such as `let`, `let*`, `letrec`,
`cond`, `case`, `when`, `unless`, `and`, `or`, `do`, `loop`, `collect`,
`assert`, `parameterize`, and `guard` are prelude or syntax-rules macros.

The runtime value surface includes integers, floats, strings, characters,
booleans, symbols, pairs/lists, vectors, hash tables, records, compiled
procedures, continuations, parameters, ports, EOF, and void. Records are
implemented as tagged hash tables generated by `define-record`. Hash-table
literals are reader sugar over `hash-table`, with mutable and functional update
helpers in the prelude. Vectors and cyclic list/vector/hash structures are
ordinary serializable values.

The standard library includes list processing, higher-order functions,
string/character utilities, bitwise operations, pseudo-random numbers, ports
and string ports, formatting/write helpers, tracing helpers, test assertions,
and process/environment helpers where the host runtime supports them. CL has
real filesystem, process, TCP, and file-watch capabilities; WASM maps persistent
file I/O to browser storage and exposes browser capabilities through JavaScript
FFI and ECE browser modules.

Error signaling is deliberately ECE-visible where practical. Undefined variable
lookup, primitive type errors, division by zero, bad archive data, missing module
imports/exports, and save/restore drift should signal ordinary ECE errors rather
than falling through to lower-level host traps. WASM has additional validation in
its archive reader and operation dispatch tables to keep malformed archive data
from becoming opaque WasmGC cast failures.
...
`compile-system` writes one archive section per input source file and concatenates them into a bundle. The bootstrap bundle is built from `src/boot-env.scm`, `src/prelude.scm`, `src/compiler.scm`, `src/reader.scm`, `src/assembler.scm`, `src/compilation-unit.scm`, `src/syntax-rules.scm`, `src/scheduler.scm`, `src/scheduler-module.scm`, `src/json.scm`, `src/json-module.scm`, `src/browser-lib.scm`, `src/browser-dom.scm`, `src/browser-html.scm`, `src/browser-canvas.scm`, `src/browser-dev.scm`, `src/wasm-host.scm`, and `src/disassemble.scm`.
...
Boot-time registration primitives bridge the self-hosted layer to both kernels.
`boot-env.scm` registers primitive IDs, assembler symbols, continuation-winding
symbols, and error symbols before the prelude, compiler, or reader are available.
On CL most of those primitives are no-ops because the runtime table already
exists; on WASM they populate the WAT-side symbol and primitive tables used by
the archive loader and executor.

## Tooling and Development Services

Installed ECE commands are selected by `argv[0]`: `ece`, `ece-repl`,
`ece-build`, and `ece-test` are dispatcher names for the same native image.
`ece-main.scm` owns command-line ordering, `--load`, `--eval`, interactive
mode, module export execution, REPL startup, and Geiser response formatting.
`ece-build.scm` packages source or archive inputs for CL and web targets, and
`ece-test.scm` runs `test-*.scm` files in fresh environments with assertion
helpers and optional name filtering.

The Geiser backend is intentionally thin. In `--geiser` mode the REPL evaluates
...
sockets and polling file-watch mtimes. ECE code owns HTTP parsing/encoding,
WebSocket frame handling, JSON envelopes, source-update policy, and cooperative
task scheduling. The dev server binds to loopback by default, serves sandbox or
template assets over HTTP, accepts browser WebSocket clients, watches the entry
file plus statically discovered load dependencies, and broadcasts whole-file
source or rebuilt archive updates. The browser evaluates received updates through
the same runtime evaluation path used by the REPL, so syntax/runtime errors are
reported without crashing the page.

The scheduler module is continuation-based rather than thread-based. Fibers run
until they yield, wait for an event, complete, or raise. `ece serve` and browser
apps use this to multiplex socket accept/read/write loops, file polling, and
frame/update events while preserving ECE's single-register-machine execution
model.
