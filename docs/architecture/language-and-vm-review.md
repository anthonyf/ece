...
**Native zones add stale-artifact failure modes.** The CL native-zone shards are generated from `bootstrap.ecec` into `.tmp/bootstrap-zones/`, listed by `manifest.sexp`, loaded before archives, then attached by `(file-stem . index)`. This is a good design for fast lookup, but stale generated native code can be semantically dangerous. The code has checks and regeneration hints, but the architecture still depends on build recipes keeping archive traversal and zone traversal in lockstep.
...
3. Start the archive-unit cleanup described in [`module-and-archive-plan.md`](module-and-archive-plan.md): route archive identity through unit IDs, split loading from instantiation, and preserve current file/global semantics while preparing for explicit modules.
