...
**Native zones add stale-artifact failure modes.** The CL native-zone bundle is generated from `bootstrap.ecec` into `.tmp/bootstrap-zones/`, loaded before archives, then attached by `(file-stem . index)`. This is a good design for fast lookup, but stale generated native code can be semantically dangerous. The code has checks and regeneration hints, but the architecture still depends on build recipes keeping archive traversal and zone traversal in lockstep.
