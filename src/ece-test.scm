...
(define (parse-test-args argv)
 ...
 (list verbose? help? paths)."
  (let loop ((rest argv)
    ...
    (cond
     ((null? rest)
      (list verbose? help? (reverse paths)))
     (else
      (let ((arg (car rest)))
        (cond
         ((or (string=? arg "-h") (string=? arg "--help"))
          (loop (cdr rest) verbose? #t paths))
         ((or (string=? arg "-v") (string=? arg "--verbose"))
          (loop (cdr rest) #t help? paths))
         ((starts-with? arg "-")
          ...
          (exit 2))
         (else
          (loop (cdr rest) verbose? help? (cons arg paths)))))))))
...
(define (run-one-test-file path verbose?)
  "Load PATH, run registered tests with per-test output capture. Returns
 (list file passes failures failure-msgs per-test-output)."
  (let ((load-error #f))
    (parameterize ((*test-state* (make-test-state)))
      ...
      (cond
       (load-error
        (list path 0 1 (list (list "(load)" load-error)) '()))
       (else
        (let ((result (run-tests)))
          ;; result = (passes failures messages per-test-output)
          (list path (car result) (cadr result) (caddr result)
                (cadddr result))))))))
...
(define (print-file-results result verbose?)
  (let ((path (car result))
        (passes (cadr result))
        (failures (caddr result))
        (messages (cadddr result))
        (per-test-output (list-ref result 4)))
    (display path)
    (display ": ")
    (display passes)
    (display " passed, ")
...
(define (print-summary all-results)
  (let loop ((rest all-results) (total-passes 0) (total-failures 0))
    (cond
     ((null? rest)
      (newline)
      (display "Total: ")
      ...
      (display " failed")
      (newline)
      (list total-passes total-failures))
     (else
      (let* ((r (car rest))
             (p (cadr r))
             (f (caddr r)))
