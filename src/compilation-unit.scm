    ...
    (let loop ((i 0) (acc '()))
      (if (= i len)
          ...
          (let ((b (char->integer (string-ref s i))))
            (bca/check-byte b "bca/string->bytes")
            (loop (+ i 1) (cons b acc)))))))
  ...
  (cond
   ...
   ((vector? datum)
    (let ((n (vector-length datum)))
      (let loop ((i 0) (acc (append (list bca/tag-vector)
                                    (bca/encode-u32 n))))
        (if (= i n)
            acc
            (loop (+ i 1)
                  (append acc (bca/encode-datum (vector-ref datum i))))))))
   ...
   (else (error "bca/encode-datum: unsupported datum" datum))))
    ...
    (cond
     ...
     ((= tag bca/tag-integer)
      (let* ((sign-result (bca/read-u8 rest))
             (mag-result (bca/read-u32 (cdr sign-result)))
             (mag (car mag-result)))
        (cons (if (= (car sign-result) 1) (- 0 mag) mag)
              (cdr mag-result))))
     ((= tag bca/tag-symbol)
...
(define (bca/encode-instruction-list instrs)
  (let loop ((rest instrs)
             (acc (bca/encode-u32 (length instrs))))
    (if (null? rest)
        acc
        (loop (cdr rest) (append acc (bca/encode-instruction (car rest)))))))
