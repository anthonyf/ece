...
(defun binary-ecec-check-count-fits (reader len context)
  (let ((remaining (binary-ecec-remaining-bytes reader)))
    (when (> len remaining)
  ...
  (let ((tag (binary-ecec-read-u8 reader "binary datum")))
    (case tag
      ...
      (4 (let ((sign (binary-ecec-read-u8 reader "binary integer")))
           (unless (or (= sign 0) (= sign 1))
             (archive-runtime-error
              "binary integer: invalid sign byte ~S." sign))
           (let ((mag (binary-ecec-read-u32 reader "binary integer")))
             (if (= sign 1) (- mag) mag))))
      ...
      (8 (let* ((len (binary-ecec-read-u32 reader "binary vector")))
           (binary-ecec-check-count-fits reader len "binary vector")
