...
(test "codegen-wasm-zone: emits lexical procedure body setup" (lambda ()
  ...
  (let* ((co (compiled-procedure-entry wasm-zone-body-target))
         (wat (generate-register-machine-wasm-zone co "zone_body")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_body\")"))
    (assert-true (string-contains? wat "h_compiled_env"))
    (assert-true (string-contains? wat "h_extend_env"))
    (assert-true (string-contains? wat "h_lexical_ref"))
    (assert-true (string-contains? wat "h_apply_primitive")))))

(test "codegen-wasm-zone: emits lexical mutation in procedure bodies" (lambda ()
  (eval-string "(define (wasm-zone-set-target x) (set! x (+ x 1)) x)")
  (let* ((co (compiled-procedure-entry wasm-zone-set-target))
         (wat (generate-register-machine-wasm-zone co "zone_set_body")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_set_body\")"))
    (assert-true (string-contains? wat "h_lexical_set"))
    (assert-true (string-contains? wat "h_lexical_ref")))))

(test "codegen-wasm-zone: emits dotted parameter constants" (lambda ()
  (eval-string "(define (wasm-zone-rest-target x . rest) rest)")
  (let* ((co (compiled-procedure-entry wasm-zone-rest-target))
         (wat (generate-register-machine-wasm-zone co "zone_rest_body")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_rest_body\")"))
    (assert-true (string-contains? wat "h_extend_env"))
    (assert-true (string-contains? wat "h_symbol_from_chars"))
    (assert-true (string-contains? wat "h_cons")))))
