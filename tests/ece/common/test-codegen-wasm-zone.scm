...
(test "codegen-wasm-zone: emits register-machine control flow" (lambda ()
  (let* ((co (mc-compile-to-code-object '(if #f 1 2)))
         (wat (generate-register-machine-wasm-zone co "zone_if_false")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_if_false\")"))
    (assert-true (string-contains? wat "flag"))
    (assert-true (string-contains? wat "dispatch"))
    (assert-true (string-contains? wat "(import \"ece\" \"h_false\""))
    (assert-true (string-contains? wat "(import \"ece\" \"h_false_p\""))
    (assert-true (string-contains? wat "h_false_p"))
    (assert-true (string-contains? wat "(br ")))))

(test "codegen-wasm-zone: emits stack and non-tail primitive apply" (lambda ()
  (let* ((co (mc-compile-to-code-object '(+ (* 2 3) 4)))
         (wat (generate-register-machine-wasm-zone co "zone_nested_prims")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_nested_prims\")"))
    (assert-true (>= (wasm-zone-test-substring-count wat "h_apply_primitive") 2))
    (assert-true (>= (wasm-zone-test-substring-count wat "h_primitive_p") 2))
    (assert-true (>= (wasm-zone-test-substring-count wat "local.set") 10))
    (assert-true (>= (wasm-zone-test-substring-count wat "stack") 10))
    (assert-true (>= (wasm-zone-test-substring-count wat "h_cons") 3))
    (assert-true (>= (wasm-zone-test-substring-count wat "h_car") 3))
    (assert-true (>= (wasm-zone-test-substring-count wat "h_cdr") 3)))))

(test "codegen-wasm-zone: emits compiled procedure call setup before bailout" (lambda ()
  (eval-string "(define (wasm-zone-call-target x) (+ x 1))")
  (let* ((co (mc-compile-to-code-object '(wasm-zone-call-target 41)))
         (wat (generate-register-machine-wasm-zone co "zone_call_setup")))
    (assert-true (wasm-zone/supported? co))
    (assert-true (string? wat))
    (assert-true (string-contains? wat "(export \"zone_call_setup\")"))
    (assert-true (string-contains? wat "h_continuation_p"))
    (assert-true (string-contains? wat "h_parameter_p"))
    (assert-true (string-contains? wat "h_compiled_entry"))
    (assert-true (string-contains? wat "h_error_sentinel_p"))
